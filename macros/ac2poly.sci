...
// e: Output scaling for the linear prediction filter.
//
// Description
// Function ac2poly() finds the best fit polynomial for FIR linear prediction filter `a`, corresponding to the autocorrelation sequence `r`. 
// `a` is the same length as `r`, and is normalized with the first element. So `a(1) = 1`.
//
// Examples
// a = ac2poly([1, 0.5, 0.2])
// [a, e] = ac2poly([1, 0.5, 0.2])
//
// See also
//  levinson
//
