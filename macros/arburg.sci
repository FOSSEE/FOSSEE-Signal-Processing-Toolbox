...
// criterion: String value, takes in "AKICc", "KIC", "AICc", "AIC", and "FPE". Default is not using a model-selection criterion.
...
elseif (nargin > 2 & ~isempty(criterion) & ...
    ...
    if (nargin > 2 & ~isempty(criterion)) then
    ...
    N = length(x);
    k = [];
    if (size(x, 1) > 1) then // if x is column vector
        f = x(2:N);
        b = x(1:N - 1);
        v = real(x' * x) / N;
